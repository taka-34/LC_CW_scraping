...
def main(cw_word,lc_word,csv_name,check):
    ...
    global fail
    log("スタートします")
    eel.view_log_js("スタートします")
    log("検索カテゴリ　ランサーズ：{}　,クラウドワークス：{}".format(lc_word,cw_word))
    eel.view_log_js("検索カテゴリ　ランサーズ：{}　,クラウドワークス：{}".format(lc_word,cw_word))
...
eel.start('index.html',options=options, close_callback=onCloseWindow, size=size, suppress_error=True)
